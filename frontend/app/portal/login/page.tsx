...
function LoginForm() {
  ...
  const [error, setError] = useState(searchParams.get('error') === 'auth_failed' ? 'Login link expired. Please sign in manually.' : '')
        ...
        <div className="bg-[#F5F0E4] p-8">
          {mode === 'login' ? (
            <>
              ...
              {error && (
                <p className="text-red-700 text-xs mb-4 bg-red-50 border border-red-200 px-3 py-2">{error}</p>
              )}
